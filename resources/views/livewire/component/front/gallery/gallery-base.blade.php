<div class=" p-10 flex justify-center items-center flex-col bg-white dark:bg-gray-900">
    <h1 class="border-b-4 text-3xl text-center text-gray-700 mb-8 dark:text-gray-300">{{(Formatting::getLang()=="en" ? "Photo Gallery" : "Galeri Foto")}}</h1>
    <div class="w-full grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 lg:gap-4 md:p-2 sm:gap-1 md:pl-20 md:pr-20">
        @if(!empty($photos))
            ...
            <a href="{{url('gallery/photo',$photo->slug)}}">
                <div class="card-zoom">
                    <img class="card-zoom-image aspect-w-16 aspect-h-9" src="{{Storage::disk('gallery_photo')->url($media[$index])}}"
                        alt="{{$photo->slug}}">
                    <h1 class="card-zoom-text">{{$photo->title}}
                    </h1>
                </div>
            </a>
            @endforeach
